...
//     const geometry::SphericalStar star(4, 1, 0.2f);
    const geometry::RocheLobeStar<unsigned short> star(4, 0.5, 1, 0.05f, 1);
    const geometry::StandardDisk<unsigned short> disk(5, 1.0f, 0.1f, 1.125f, 1e-3);
    const geometry::DiskBelt<unsigned short> belt(disk);
    const std::vector< geometry::ObjectModel<unsigned short> >  oms { star.get_object_model(),  disk.get_object_model(),  belt.get_object_model() };
